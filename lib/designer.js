...
function populateFieldSelectionPanel() {
    ...
    const fieldPanelTableSelect = document.getElementById("fieldPanelTableSelect");
    fieldPanelTableSelect.innerHTML = "<option value=''>" + t("designerSelectSource") + "</option>";
    populateTableSelect(fieldPanelTableSelect, t("designerSelectTable"), constructorMode === "report");//
    // 1. Додаємо звичайні таблиці
    database.tables.forEach(table => {
        const option = document.createElement("option");
        option.value = table.name;
        option.textContent = table.name;
    ...
    // 2. Додаємо результати виконаних запитів (для форм і звітів)
    queries.results.forEach(query => {
        const option = document.createElement("option");
        // Формуємо ім'я як *запит "Назва", щоб суміщати з логікою previewForm
        const internalName = query.name.startsWith('запит "') ? query.name : `запит "${query.name}"`;
        option.value = `*${internalName}`;
        option.textContent = t("designerQueryResult", internalName);
        fieldPanelTableSelect.appendChild(option);
    });

    // 3. Додаємо визначення запитів (якщо результати ще не згенеровані)
    queries.definitions.forEach(query => {
        const option = document.createElement("option");
        const internalName = `запит "${query.name}"`;
        option.value = `*${internalName}`;
        option.textContent = t("designerQueryDef", internalName);
        fieldPanelTableSelect.appendChild(option);
    });
